...
AudioEngine::AudioEngine() : loopsPlaying(), soundBanks(), eventDescriptions(), eventInstances() {}
...
void AudioEngine::loadFMODStudioEvent(const char* eventName,
                                      std::vector<std::pair<const char*, float>> paramsValues) {
...
void AudioEngine::setFMODEventParamValue(const char* eventName, const char* parameterName,
                                         float value) {
...
void AudioEngine::printEventInfo(FMOD::Studio::EventDescription* eventDescription) {
    ...
    //           << (is3D ? " is " : " isn't ") << " 3D, " << (isOneshot ? " is " : " isn't ")
    //           << " oneshot, " << (eventDescription->isValid() ? " is " : " isn't ") << " valid."
    //           << '\n';
}
